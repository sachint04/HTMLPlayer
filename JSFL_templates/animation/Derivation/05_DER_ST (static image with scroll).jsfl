...
function saveXML(){
	fl.trace("saveXML()");
	sXMLData = "<data pageType='DER_ST'>" + sComponentXML + sSoundsXML + "</data>";
	//fl.trace(sXMLData);
	if(!URIExists(sSaveDir + sDocumentName)){
		createFolder(sSaveDir + sDocumentName);
	}
...
function init(){
	...
	sComponentXML += '<component type="derivationpanel_static" componentID="derivationpanelcontainer" view="derivation_panel_static.html" viewLocation="global_html"></component>';
	saveXML();
	
	sHTMLData = '<!-- The ID below for the root div tag will be replaced dynamically by the Pages GUID -->';
	sHTMLData += '<div id="content" class="page-content DER STATIC image-page">';
	...
	sHTMLData += '</div>';
	sHTMLData += '</div>';
	sHTMLData += '<div id="derivationpanelcontainer" class="static">';
	sHTMLData += '</div>';
	sHTMLData += '</div>';
	sHTMLData += '</div>';
	saveHTML(sHTMLData);
