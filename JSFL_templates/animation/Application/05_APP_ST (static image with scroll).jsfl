...
function addItemToStage(p_sItemName){
	...
	if(oLibrary.itemExists(p_sItemName)){
		...
		var nMaxWidth = Math.round(oDocument.getElementProperty("width")),
			nMaxHeight = Math.round(oDocument.getElementProperty("height"));
...
function saveXML(){
	fl.trace("saveXML()");
	sXMLData = "<data pageType='APP_ST'>" + sComponentXML + sSoundsXML + "</data>";
	//fl.trace(sXMLData);
	if(!URIExists(sSaveDir + sDocumentName)){
		createFolder(sSaveDir + sDocumentName);
	}
...
function init(){
	...
	sComponentXML += '<component type="applicationpanel_static" componentID="applicationpanelcontainer" view="application_panel_static.html" viewLocation="global_html"></component>';
	saveXML();
	
	sHTMLData = '<!-- The ID below for the root div tag will be replaced dynamically by the Pages GUID -->';
	sHTMLData += '<div id="content" class="page-content APP STATIC image-page">';
	...
	sHTMLData += '</div>';
	sHTMLData += '</div>';
	sHTMLData += '<div id="applicationpanelcontainer" class="static">';
	sHTMLData += '</div>';
	sHTMLData += '</div>';
	sHTMLData += '</div>';
	saveHTML(sHTMLData);
